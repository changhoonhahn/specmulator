...
def HOD_LHD_test(HODrange=None, samples=None, overwrite=False, seed=1):
    ...
    fname = ''.join([UT.dat_dir(), 'lhd/HOD_LHD.', HODrange, '.rseed', str(seed), '.', str(samples), 'samples_test.dat']) 
    
    if os.path.isfile(fname) and not overwrite: # file exists 
        theta_test = np.loadtxt(fname, skiprows=4)
    else: 
        theta_test = np.zeros((samples, len(HOD_range_min)))
    
        np.random.seed(seed)
        for i in range(theta_test.shape[1]):
            dHOD_range = HOD_range_max[i] - HOD_range_min[i] 
            ti = np.linspace(HOD_range_min[i] + 0.25*dHOD_range, HOD_range_max[i]-0.25*dHOD_range, theta_test.shape[0])
